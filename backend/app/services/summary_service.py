...
def extract_parties(text: str) -> tuple[str, str]:
    ...
    # Normalize alef ONLY for matching (1:1 replacement → spans stay valid on original)
    header_normalized = re.sub(r"[أإآ]", "ا", header)
    ...
    match_a = re.search(pattern_a, header_normalized)
    match_b = re.search(pattern_b, header_normalized)

    if match_a:
        # Extract from ORIGINAL header using span (safe: alef norm is 1:1)
        start, end = match_a.span(2)
        name = header[start:end].strip()
        prefix = ""
        if match_a.group(1):
            ps, pe = match_a.span(1)
            prefix = header[ps:pe].strip()
        if name and len(name) > 3:
            name = re.split(r"(المشار|بصفته|المقيم|وجنسيته|سوري|من|المسمى)", name)[
                0].strip()
            party_a_name = f"{prefix} {name}".strip()

    if match_b:
        start, end = match_b.span(2)
        name = header[start:end].strip()
        prefix = ""
        if match_b.group(1):
            ps, pe = match_b.span(1)
            prefix = header[ps:pe].strip()
        if name and len(name) > 3:
            name = re.split(r"(المشار|بصفته|المقيم|وجنسيته|سوري|من|المسمى)", name)[
                0].strip()
...
def generate_contract_summary(text: str, classified_clauses: List[Dict[str, Any]]) -> str:
    ...
    high_risk_count = sum(1 for c in classified_clauses if c.get(
        "predicted_risk_level", "").lower() == "high")
    medium_risk_count = sum(1 for c in classified_clauses if c.get(
        "predicted_risk_level", "").lower() == "medium")
    ...
    sentences.append(
        f"تم تحليل هذا العقد الذي يجمع بين كل من {party_a} (كالطرف الأول) و {party_b} (كالطرف الثاني).")
    ...
    sentences.append(
        f"يتكون العقد من {total_clauses} بنداً رئيسياً تم فحصها{financial_str}{termination_str}.")
    ...
    if high_risk_count > 0 or medium_risk_count > 0:
        ...
        sentences.append(
            f"أظهر التحليل وجود {' و '.join(risk_detail)} تتطلب المراجعة والانتباه.")
    else:
        sentences.append(
            "لم يتم العثور على أي بنود عالية أو متوسطة الخطورة، وتعتبر بنود العقد متوازنة ومستقرة.")

    # Sentence 4: Key high-risk warnings if any
    high_risk_clauses = [c for c in classified_clauses if c.get(
        "predicted_risk_level", "").lower() == "high"]
    if high_risk_clauses:
        ...
        if warnings:
            sentences.append("أبرز المخاطر المكتشفة: " +
                             " بالإضافة إلى " .join(warnings))

    # Sentence 5: Call to action
    sentences.append(
        "يوصى بمراجعة البنود المظللة باللون الأحمر بدقة ومناقشة تعديلها مع الطرف الآخر قبل توقيع العقد رسمياً.")
